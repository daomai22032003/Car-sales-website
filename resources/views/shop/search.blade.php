                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb" style="background: none; padding-left: 0;">
                        <li class="breadcrumb-item">
                            <a href="/" style="color: #1e88e5;">Trang chủ</a>
                        </li>
                ...
                <div class="search-header"
                    ...
                    <h1 style="font-weight: 800; color: #1a2a5a; font-size: 28px;">
                        ...
                        <span style="color: #b02a2a;">
                            ...
                            {{-- HÃNG XE --}}
                            @if($category)
                                Hãng xe: {{ $category->name }}
                            @endif

                            {{-- GIÁ --}}
                            @if(request('price'))
                                @if($category) - @endif
                                Giá: {{ str_replace('-', ' - ', request('price')) }} triệu
                            @endif

                            {{-- TỪ KHÓA --}}
                            @if(request('keyword'))
                                @if($category || request('price')) - @endif
                                "{{ request('keyword') }}"
                            @endif
                    ...
                    <p style="color: #666; font-size: 16px;">
                        Tìm thấy <strong>{{ $totalResult }}</strong> sản phẩm phù hợp.
                    </p>
        ...
        <div class="row">
            ...
            @else
                <div class="col-12 text-center" style="padding: 100px 0;">
                    ...
                    <p style="color: #999;">
                        Thử tìm kiếm với từ khóa khác hoặc hãng xe khác.
                    </p>

                    <a href="/"
                       class="btn btn-primary"
                       style="margin-top: 20px; background: #2196f3; border: none; padding: 10px 30px; border-radius: 10px;">
                        Quay lại trang chủ
                    </a>
                </div>
            @endif
